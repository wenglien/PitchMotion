...
# S4: Arm angular velocity (shoulder→wrist rotation rate)
S4_DEFAULT_WEIGHT    = 0.35   # S4 信號權重（略低於 S1 的 0.4）
S4_ADVANCE_SEC       = 0.020  # 角速度峰值到放球的補償（比 S1 的 0.008 大，峰值稍早）
S4_SEARCH_START_FRAC = 0.25   # 從後 75% 開始搜尋（投球後期）
S4S1_AGREEMENT_SEC   = 0.20   # S4-S1 不一致閾值（秒），超過則降低 S4 權重
S4_DOWNWEIGHTED      = 0.10   # S4 與 S1 不一致時的降低權重
...
class ReleasePointDetector:
    ...
    def _detect_signal_s4_arm_angular_velocity(self) -> Optional[int]:
        """
        訊號 S4：手臂角速度峰值（shoulder→wrist 向量旋轉速率）。

        棒球投球的甩臂動作會在出球前產生最大角速度。
        相比 S1（手腕線速度），角速度對攝影機視角不敏感、
        對手腕遮蔽容忍度高，且對各種投法（速球/側投/下投）都有效。
        ...
        for i, frame_data in enumerate(self.pose_history):
            ...
            if not shoulder or not wrist:
                continue
            if (shoulder.get("visibility", 1.0) < MIN_VISIBILITY
                    or wrist.get("visibility", 1.0) < MIN_VISIBILITY):
                continue

            dx = float(wrist["x"]) - float(shoulder["x"])
            dy = float(wrist["y"]) - float(shoulder["y"])
            angle = float(np.arctan2(dy, dx))   # 弧度，-π ~ π
            angles.append(angle)
            valid_indices.append(i)

        if len(angles) < MIN_FRAMES_FOR_DETECTION:
            log.debug("S4: not enough valid arm angle frames (%d)", len(angles))
            return None

        # ── 2. 相位解卷繞（避免 -π/π 跳躍造成假峰值）────────────────────
        angles_unwrapped = list(np.unwrap(angles))

        # ── 3. 計算角速度（弧度/幀），使用中心差分 + 移動平均平滑 ────────
        n = len(angles_unwrapped)
        raw_ang_vel: list[float] = []
        for i in range(n):
            if i == 0:
                dtheta = angles_unwrapped[1] - angles_unwrapped[0]
            elif i == n - 1:
                dtheta = angles_unwrapped[n-1] - angles_unwrapped[n-2]
            else:
                dtheta = (angles_unwrapped[i+1] - angles_unwrapped[i-1]) / 2.0
            raw_ang_vel.append(abs(dtheta))   # 取絕對值（只關心旋轉速率）

        # 移動平均平滑（窗口與 S1 一致）
        vel_window = max(3, int(round(self.fps / VEL_WINDOW_DIVISOR)))
        ang_vel: list[float] = []
        for i in range(n):
            s = max(0, i - vel_window // 2)
            e = min(n, i + vel_window // 2 + 1)
            ang_vel.append(float(np.mean(raw_ang_vel[s:e])))

        # ── 4. 在後段搜尋角速度峰值（投球後期）──────────────────────────
        search_start = int(n * S4_SEARCH_START_FRAC)
        if search_start >= n:
            search_start = 0

        peak_local_idx = search_start + int(np.argmax(ang_vel[search_start:]))

        # ── 5. 前移補償（峰值到放球的時間差）────────────────────────────
        offset = -int(round(S4_ADVANCE_SEC * self.fps))
        chosen_local_idx = max(0, min(n - 1, peak_local_idx + offset))
        frame_idx = int(valid_indices[chosen_local_idx])

        log.debug(
            "S4 arm angular velocity: frame=%d (peak_local=%d, offset=%d, ang_vel_peak=%.4f rad/frame)",
            frame_idx, peak_local_idx, offset, ang_vel[peak_local_idx],
        )
        return frame_idx
    ...
    def detect_release_point(self, first_ball_frame: Optional[int] = None) -> Optional[Dict]:
        ...
        # Pose 品質檢查：若有效（非 None）幀數太少，訊號可靠度不足。
        # 直接回傳 None 避免低品質 Pose 訊號誤導出球點。
        valid_pose_frames = sum(1 for f in self.pose_history if f is not None)
        valid_pose_ratio = valid_pose_frames / len(self.pose_history)
        if valid_pose_ratio < 0.30:
            log.info(
                "Pose quality too low (%.0f%% valid frames, %d/%d) — skipping release detection",
                valid_pose_ratio * 100, valid_pose_frames, len(self.pose_history),
            )
            return None
        ...
        s4_frame = self._detect_signal_s4_arm_angular_velocity()

        signals = {
            's1_wrist_speed':       s1_frame,
            's2_elbow_extension':   s2_frame,
            's3_foot_window':       s3_window,
            's4_arm_angular_vel':   s4_frame,
        }
        ...
        # ---- S4-S1 一致性檢查 ----
        s4_weight = S4_DEFAULT_WEIGHT
        if s4_frame is not None and s1_frame is not None:
            gap_s4s1 = abs(s4_frame - s1_frame)
            if gap_s4s1 > int(round(S4S1_AGREEMENT_SEC * self.fps)):
                s4_weight = S4_DOWNWEIGHTED
                log.debug("S4-S1 disagreement (gap=%d frames), downweighting S4", gap_s4s1)
        ...
        if s4_frame is not None:
            candidates.append((s4_frame, s4_weight))
        ...
        if s3_window is not None:
            ...
            # else: S3 約束失敗時不丟棄所有候選——繼續使用 S1/S2/S4
        ...
        num_signals = sum(1 for s in [s1_frame, s2_frame, s4_frame] if s is not None)
        if num_signals <= 1:
            confidence = min(confidence, SINGLE_SIGNAL_MAX_CONF)
        # Pose 品質不佳時按比例降低信心（有效幀率 < 60% 開始降低）
        if valid_pose_ratio < 0.60:
            confidence *= valid_pose_ratio / 0.60
        confidence = min(confidence, 1.0)

        log.info(
            "Release point: frame=%d, confidence=%.2f, signals(S1=%s, S2=%s, S3=%s, S4=%s)",
            best_frame, confidence, s1_frame, s2_frame, s3_window, s4_frame,
        )
        return {
